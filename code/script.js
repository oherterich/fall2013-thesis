...
//         get: 'tagged',
//         tagName: 'puppy',
//         clientId: '3a4831a9152845bcb5caa94f713a8d00',
//         limit: '60',
//         links: false,
//         resolution: 'standard_resolution',
//         template: '<a href="{{image}}" class="instagram-photo-link"></a>'
//     });
//     feed.run();

window.onload = function() { //THIS FUNCTION WRAPS AROUND THE REST OF THE CODE. REMEMBER THIS.
    
    // var photoLinks = document.querySelectorAll('.instagram-photo-link');
    // for (var i = 0; i < photoLinks.length; i++) {
    // 	var p = photoLinks.item(i).href;
    // 	photos.push(p);
    // }
...
//Boolean that switches the fading on and off
var fade = false;
...
function animate(t) {
	...
	//console.log(camera.position.z + " " + clickInfo.x);

	if (fade){
		//These two variables determine the fade.
		var range = map_range(camera.position.z, 1000, 4500, 1200/2, 3500/2); //used to detect which planes are in the spotlight
		var strength = map_range(camera.position.z, 1000, 4500, 0.003, 0.0005); //used to determine how fast the planes fade
		...
		for (var i = 0; i < planeList.length; i++) {
			...
			if (planeList[i].position.x > (directionVector.x - SCREEN_WIDTH / 2 - range) && planeList[i].position.x < (directionVector.x - SCREEN_WIDTH / 2 + range)) {
				if (planeList[i].position.y > (directionVector.y + SCREEN_HEIGHT / 2 - range) && planeList[i].position.y < (directionVector.y + SCREEN_HEIGHT / 2 + range)) {
					...
					planeList[i].material.color.g -= strength;
					planeList[i].material.color.b -= strength;

					// planeList[i].material.color.r = 1;
					// planeList[i].material.color.g = 1;
					// planeList[i].material.color.b = 1;
				}
			}
			// else {
			// 	planeList[i].material.color.r = 1;
			// 	planeList[i].material.color.g = 0;
			// 	planeList[i].material.color.b = 0;
			// }
		}
	}
